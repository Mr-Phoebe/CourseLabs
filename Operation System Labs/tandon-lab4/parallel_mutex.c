...
Mutex performs better than spin. Because it's better to put threads
to sleep and swap immediately rather than to keep waiting until it gets released.
The only thing that makes Mutex slow is the overhead of swapping.


Part 3:
Retrieving an item from the hash table does not require a lock.
Just like the shared lock in Two Phases Lock in DBMS. The reading
operation is mutually exclusive with writing operation. But in
this case, the writing operation will not effect with reading
operation. So there is no need to add lock for reading operation(retrieving an item).
*/
